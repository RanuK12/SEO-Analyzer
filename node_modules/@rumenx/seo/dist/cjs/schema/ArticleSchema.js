...
const BaseSchema_1 = require("./BaseSchema.js");
/**
 ...
 * ```
 */
class ArticleSchema extends BaseSchema_1.BaseSchema {
    getType() {
        return 'Article';
    }
    /**
     ...
     */
    setAuthors(authors) {
        const authorSchemas = authors.map(author => typeof author === 'string' ? { '@type': 'Person', name: author } : author);
     ...
     */
    addComment(comment) {
        const comments = this.getProperty('comment') || [];
    ...
    validate() {
        super.validate();
        const headline = this.getProperty('headline');
        if (!headline) {
    ...
    static fromData(data) {
        ...
        if (data.datePublished)
            schema.setDatePublished(data.datePublished);
        if (data.dateModified)
            schema.setDateModified(data.dateModified);
        if (data.image)
            schema.setImage(data.image);
        if (data.publisher)
            schema.setPublisher(data.publisher);
        if (data.url)
            schema.setUrl(data.url);
        if (data.articleBody)
            schema.setArticleBody(data.articleBody);
        if (data.wordCount)
            schema.setWordCount(data.wordCount);
        if (data.keywords)
            schema.setKeywords(data.keywords);
        if (data.articleSection)
            schema.setArticleSection(data.articleSection);
        if (data.inLanguage)
            schema.setInLanguage(data.inLanguage);
        return schema;
    }
}
