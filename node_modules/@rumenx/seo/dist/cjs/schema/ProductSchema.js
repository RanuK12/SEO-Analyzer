 ...
 * ```
 */
class ProductSchema extends BaseSchema_1.BaseSchema {
    getType() {
        return 'Product';
    }
    /**
     ...
     * @returns This instance for chaining
     */
    setPrice(price, currency, priceValidUntil) {
        const offer = {
            '@type': 'Offer',
    ...
    setAvailability(availability) {
        const offer = this.getProperty('offers') || { '@type': 'Offer' };
    ...
    setUrl(url) {
        ...
        const offer = this.getProperty('offers');
        if (offer) {
            offer.url = url;
     ...
     * @returns This instance for chaining
     */
    setAggregateRating(ratingValue, reviewCount, bestRating = 5, worstRating = 1) {
     ...
     */
    addReview(review) {
        const reviews = this.getProperty('review') || [];
        if ('@type' in review) {
            reviews.push(review);
        }
        else {
    ...
    static fromData(data) {
        ...
        if (data.sku)
            schema.setSku(data.sku);
        if (data.brand)
            schema.setBrand(data.brand);
        if (data.price && data.currency)
            schema.setPrice(data.price, data.currency);
        if (data.availability)
            schema.setAvailability(data.availability);
        if (data.url)
            schema.setUrl(data.url);
        if (data.gtin)
            schema.setGtin(data.gtin);
        if (data.mpn)
            schema.setMpn(data.mpn);
        return schema;
    }
}
