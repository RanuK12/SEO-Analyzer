...
class AiSeoAnalyzer {
    constructor(config = {}) {
        ...
        Object.defineProperty(this, "processingStats", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: {
     ...
     */
    analyzeSeo(contentAnalysis, seoResult, options = {}) {
     ...
     */
    determineFocusAreas(contentAnalysis, seoResult) {
        const focusAreas = [];
        // Content issues
     ...
     */
    generateSuggestionsByType(contentAnalysis, seoResult, type) {
        const suggestions = [];
        switch (type) {
            case 'technical':
     ...
     */
    generateTechnicalSuggestions(seoResult) {
        const suggestions = [];
     ...
     */
    generateContentSuggestions(contentAnalysis, _seoResult) {
        const suggestions = [];
        if (contentAnalysis.wordCount < 300) {
            suggestions.push({
                id: 'content-length-1',
     ...
     */
    generateKeywordSuggestions(seoResult) {
        const suggestions = [];
        if (!seoResult.keywords || seoResult.keywords.length < 5) {
            suggestions.push({
                ...
                description: 'Limited keyword presence detected. Focus on relevant keywords throughout your content.',
                priority: 'high',
                actionSteps: [
     ...
     */
    generateStructureSuggestions(contentAnalysis) {
        const suggestions = [];
        ...
        if (h1Headings.length !== 1) {
            suggestions.push({
                ...
                description: h1Headings.length === 0
                    ? 'Missing H1 heading.'
     ...
     */
    generateMetaSuggestions(seoResult) {
        const suggestions = [];
        ...
        if (!seoResult.description || seoResult.description.length < 50) {
            suggestions.push({
                ...
                description: 'Meta description is missing or too short. Add a compelling description for search results.',
                priority: 'high',
     ...
     */
    generateImageSuggestions(contentAnalysis) {
        const suggestions = [];
        const imagesWithoutAlt = contentAnalysis.structure.images.filter(img => !img.currentAlt || img.currentAlt.trim() === '');
     ...
     */
    generateLinkSuggestions(contentAnalysis) {
        const suggestions = [];
    ...
    calculateTotalImpact(suggestions) {
        ...
        if (impacts.some(i => i.toLowerCase().includes('critical')))
            return 'Critical impact';
        if (impacts.some(i => i.toLowerCase().includes('high')))
            return 'High impact';
        if (impacts.some(i => i.toLowerCase().includes('medium')))
     ...
     */
    calculateOverallScore(suggestions, _seoResult) {
     ...
     */
    extractTopRecommendations(suggestions) {
        return suggestions
...
exports.AiSeoAnalyzer = AiSeoAnalyzer;
//# sourceMappingURL=AiSeoAnalyzer.js.map